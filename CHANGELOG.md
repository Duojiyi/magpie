...
> 主题：**macOS 与 Linux 真正可用**。此前这两个平台「能装能启动」，但剪贴板核心是一组空实现——复制图片、复制文件、粘贴富文本都会报成功却什么都没发生。本版把它们换成了真实现，并首次提供 Linux 安装包。Windows 行为不变。
>
> ⚠️ **macOS / Linux 标记为 beta**：全部改动经三平台 CI 编译与单元测试验证，但**尚未在真实 macOS / Linux 机器上人工验证过**。已知缺口见下方「本版尚未覆盖」。Windows 版不受影响，可照常升级。

### 新增

- **Linux 支持**：首次提供 `.deb` 与 `.AppImage` 安装包（x86_64）。
- **macOS / Linux 剪贴板真实现**：文本、HTML 富文本、图片、文件列表的读写全部接通（macOS 走 NSPasteboard，Linux 走 X11），替换掉原先返回「成功」却无任何行为的空实现。
...
这些能力在这两个平台上仍缺失或降级，会在后续版本补齐：

- **来源应用识别**：历史条目的来源应用恒为占位值，导致来源图标、按应用清理规则、富文本/表格来源启发式失效。
- **全局键盘导航**：面板未获焦点时的方向键选择、修饰键+数字快捷粘贴不可用（依赖 Windows 低级键盘钩子）。
- **Wayland**：输入注入在原生 Wayland 会话下无效（XWayland 可用）；注销时可能产生一次崩溃报告。
- **动图**：GIF 会退化为静态首帧；Office 私有剪贴板格式不做往返保留。
- **静态加密**：非 Windows 无 DPAPI 对应物，敏感数据目前明文落库。
